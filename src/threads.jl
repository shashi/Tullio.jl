...
@inline function threader(fun!, T::Type, Z::AbstractArray, As::Tuple, I0s::Tuple, J0s::Tuple, redfun, block, keep=nothing)
    ...
    elseif length(Z) == 1 && eltype(Z) <: Number
        thread_scalar(fun!, T, Z, As, Js, redfun, block, Threads.nthreads(), keep)
    else
        fun!(T, Z, As..., Is..., Js..., keep)
    end
    return nothing
...
function thread_scalar(fun!, T::Type, Z::AbstractArray, As::Tuple, Js::Tuple, redfun, block::Int, spawns::Int, keep=nothing)
    if productlength(Js) <= block || spawns < 2
        ...
        J1s, J2s = cleave(Js)
        Base.@sync begin
            Threads.@spawn thread_scalar(fun!, T, Z1, As, J1s, redfun, block, div(spawns,2))
            Threads.@spawn thread_scalar(fun!, T, Z2, As, J2s, redfun, block, div(spawns,2))
        end
        if keep === nothing
            Z .= redfun.(Z1, Z2)
        else
            Z .= redfun.(Z, redfun.(Z1, Z2))
        end
    end
    nothing
end
